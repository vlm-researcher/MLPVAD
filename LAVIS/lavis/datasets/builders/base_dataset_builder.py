...
class BaseDatasetBuilder:
    ...
    def _download_ann(self):
        ...
        for split in splits:
            ...
            if "url" not in info:
                if not os.path.exists(info["storage"]):
                    raise FileNotFoundError(
                        f"Annotation file for split '{split}' not found at: {info['storage']}"
                    )
                else:
                    logging.info(f"Using local annotation for '{split}' from: {info['storage']}")
                continue
    ...
    def _download_vis(self):
        storage_path = self.config.build_info.get(self.data_type).train.storage
        print("Image path: ", storage_path)
    ...
    def build(self):
        ...
        for split in ann_info.keys():
            ...
            print("Dataset Info: ", dataset_cls, vis_processor, text_processor)
